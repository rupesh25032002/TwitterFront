...
export const Profile = () => {
  ...
  const getCurrentUserData = async () => {
    try {
      const res = await axios.get(`/api/user/getuser/${userId?.id}`);
  ...
  const getUserTweets = async () => {
    try {
      const res = await axios.post(`/api/tweet/user/all/${userId?.id}`);
      console.log(res);
    ...
    try {
      if (!isFollow) {
        const res = await axios.post(
          `/api/user/follow/${currentUserDetail?._id}`,
          {
        ...
        const res = await axios.post(
          `/api/user/unfollow/${currentUserDetail?._id}`,
          {
            userId: `${userDetail?._id}`,
          }
        );
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      const res = await axios.put(
        `/api/user/update/${currentUserDetail?._id}`,
        {
          ...updateDetail,
        }
      );
